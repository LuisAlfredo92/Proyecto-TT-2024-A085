...
public class DigitalDataEmailCamelliaFpeTests
{
    ...
    [Benchmark]
    public char[][] EncryptEmailCamelliaFpe()
    {
        var encryptedEmail = new char[_emails.Length][];
        for (var i = 0; i < _emails.Length; i++)
        {
            encryptedEmail[i] = _camelliaFpe.Encrypt(_emails[i]);
        }
    ...
    public void SetupDecryption()
    {
        ...
        {
            _emails[i] = _camelliaFpe.Encrypt(emails[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptEmailCamelliaFpe()
    {
        var decryptedEmail = new char[_emails.Length][];
        for (var i = 0; i < _emails.Length; i++)
        {
            decryptedEmail[i] = _camelliaFpe.Decrypt(_emails[i]);
        }
