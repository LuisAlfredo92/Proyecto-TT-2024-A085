...
public class LaborDataPositionSerpentFpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptPositionSerpentFpe))]
    ...
    [Benchmark]
    public char[][] EncryptPositionSerpentFpe()
    {
        var encryptedPosition = new char[_positions.Length][];
        for (var i = 0; i < _positions.Length; i++)
        {
            encryptedPosition[i] = _serpentFpe.Encrypt(_positions[i]);
        }

        return encryptedPosition;
    }

    [GlobalSetup(Target = nameof(DecryptPositionSerpentFpe))]
    public void SetupDecryption()
    {
        ...
        {
            _positions[i] = _serpentFpe.Encrypt(companies[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptPositionSerpentFpe()
    {
        var decryptedPosition = new char[_positions.Length][];
        for (var i = 0; i < _positions.Length; i++)
        {
            decryptedPosition[i] = _serpentFpe.Decrypt(_positions[i]);
        }
