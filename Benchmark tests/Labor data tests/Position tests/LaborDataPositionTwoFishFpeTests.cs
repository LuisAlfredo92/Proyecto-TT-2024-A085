...
public class LaborDataPositionTwoFishFpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptPositionTwoFishFpe))]
    ...
    [Benchmark]
    public char[][] EncryptPositionTwoFishFpe()
    {
        var encryptedPosition = new char[_positions.Length][];
        for (var i = 0; i < _positions.Length; i++)
        {
            encryptedPosition[i] = _twoFishFpe.Encrypt(_positions[i]);
        }

        return encryptedPosition;
    }

    [GlobalSetup(Target = nameof(DecryptPositionTwoFishFpe))]
    public void SetupDecryption()
    {
        ...
        {
            _positions[i] = _twoFishFpe.Encrypt(companies[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptPositionTwoFishFpe()
    {
        var decryptedPosition = new char[_positions.Length][];
        for (var i = 0; i < _positions.Length; i++)
        {
            decryptedPosition[i] = _twoFishFpe.Decrypt(_positions[i]);
        }
