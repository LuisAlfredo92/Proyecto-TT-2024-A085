...
public class LaborDataPositionCast256FpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptPositionCast256Fpe))]
    ...
    [Benchmark]
    public char[][] EncryptPositionCast256Fpe()
    {
        var encryptedPosition = new char[_positions.Length][];
        for (var i = 0; i < _positions.Length; i++)
        {
            encryptedPosition[i] = _cast256Fpe.Encrypt(_positions[i]);
        }

        return encryptedPosition;
    }

    [GlobalSetup(Target = nameof(DecryptPositionCast256Fpe))]
    public void SetupDecryption()
    {
        ...
        {
            _positions[i] = _cast256Fpe.Encrypt(companies[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptPositionCast256Fpe()
    {
        var decryptedPosition = new char[_positions.Length][];
        for (var i = 0; i < _positions.Length; i++)
        {
            decryptedPosition[i] = _cast256Fpe.Decrypt(_positions[i]);
        }
