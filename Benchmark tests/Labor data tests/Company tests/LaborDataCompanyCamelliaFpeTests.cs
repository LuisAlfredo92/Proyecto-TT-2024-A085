...
public class LaborDataCompanyCamelliaFpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptCompanyCamelliaFpe))]
    ...
    [Benchmark]
    public char[][] EncryptCompanyCamelliaFpe()
    {
        var encryptedCompany = new char[_companies.Length][];
        for (var i = 0; i < _companies.Length; i++)
        {
            encryptedCompany[i] = _camelliaFpe.Encrypt(_companies[i]);
        }

        return encryptedCompany;
    }

    [GlobalSetup(Target = nameof(DecryptCompanyCamelliaFpe))]
    public void SetupDecryption()
    {
        ...
        {
            _companies[i] = _camelliaFpe.Encrypt(companies[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptCompanyCamelliaFpe()
    {
        var decryptedCompany = new char[_companies.Length][];
        for (var i = 0; i < _companies.Length; i++)
        {
            decryptedCompany[i] = _camelliaFpe.Decrypt(_companies[i]);
        }
