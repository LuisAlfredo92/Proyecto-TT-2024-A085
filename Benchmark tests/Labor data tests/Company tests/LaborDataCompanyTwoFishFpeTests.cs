...
public class LaborDataCompanyTwoFishFpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptCompanyTwoFishFpe))]
    ...
    [Benchmark]
    public char[][] EncryptCompanyTwoFishFpe()
    {
        var encryptedCompany = new char[_companies.Length][];
        for (var i = 0; i < _companies.Length; i++)
        {
            encryptedCompany[i] = _twoFishFpe.Encrypt(_companies[i]);
        }

        return encryptedCompany;
    }

    [GlobalSetup(Target = nameof(DecryptCompanyTwoFishFpe))]
    public void SetupDecryption()
    {
        ...
        {
            _companies[i] = _twoFishFpe.Encrypt(companies[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptCompanyTwoFishFpe()
    {
        var decryptedCompany = new char[_companies.Length][];
        for (var i = 0; i < _companies.Length; i++)
        {
            decryptedCompany[i] = _twoFishFpe.Decrypt(_companies[i]);
        }
