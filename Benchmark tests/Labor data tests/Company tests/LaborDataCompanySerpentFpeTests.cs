...
public class LaborDataCompanySerpentFpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptCompanySerpentFpe))]
    ...
    [Benchmark]
    public char[][] EncryptCompanySerpentFpe()
    {
        var encryptedCompany = new char[_companies.Length][];
        for (var i = 0; i < _companies.Length; i++)
        {
            encryptedCompany[i] = _serpentFpe.Encrypt(_companies[i]);
        }

        return encryptedCompany;
    }

    [GlobalSetup(Target = nameof(DecryptCompanySerpentFpe))]
    public void SetupDecryption()
    {
        ...
        {
            _companies[i] = _serpentFpe.Encrypt(companies[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptCompanySerpentFpe()
    {
        var decryptedCompany = new char[_companies.Length][];
        for (var i = 0; i < _companies.Length; i++)
        {
            decryptedCompany[i] = _serpentFpe.Decrypt(_companies[i]);
        }
