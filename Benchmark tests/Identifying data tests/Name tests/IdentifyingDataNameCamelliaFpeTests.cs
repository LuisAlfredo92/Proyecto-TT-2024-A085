...
public class IdentifyingDataNameCamelliaFpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptNameCamelliaFpe))]
    ...
    [Benchmark]
    public char[][] EncryptNameCamelliaFpe()
    {
        var encryptedName = new char[_names.Length][];
        for (var i = 0; i < _names.Length; i++)
        {
            encryptedName[i] = _camelliaFpe.Encrypt(_names[i]);
        }

        return encryptedName;
    }

    [GlobalSetup(Target = nameof(DecryptNameCamelliaFpe))]
    public void SetupDecryption()
    {
        ...
        {
            _names[i] = _camelliaFpe.Encrypt(names[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptNameCamelliaFpe()
    {
        var decryptedName = new char[_names.Length][];
        for (var i = 0; i < _names.Length; i++)
        {
            decryptedName[i] = _camelliaFpe.Decrypt(_names[i]);
        }
