...
public class IdentifyingDataNameTwoFishFpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptNameTwoFishFpe))]
    ...
    [Benchmark]
    public char[][] EncryptNameTwoFishFpe()
    {
        var encryptedName = new char[_names.Length][];
        for (var i = 0; i < _names.Length; i++)
        {
            encryptedName[i] = _twoFishFpe.Encrypt(_names[i]);
        }

        return encryptedName;
    }

    [GlobalSetup(Target = nameof(DecryptNameTwoFishFpe))]
    public void SetupDecryption()
    {
        ...
        {
            _names[i] = _twoFishFpe.Encrypt(names[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptNameTwoFishFpe()
    {
        var decryptedName = new char[_names.Length][];
        for (var i = 0; i < _names.Length; i++)
        {
            decryptedName[i] = _twoFishFpe.Decrypt(_names[i]);
        }
