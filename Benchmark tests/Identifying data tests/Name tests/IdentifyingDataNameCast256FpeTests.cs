...
public class IdentifyingDataNameCast256FpeTests
{
    ...
    [GlobalSetup(Target = nameof(EncryptNameCast256Fpe))]
    ...
    [Benchmark]
    public char[][] EncryptNameCast256Fpe()
    {
        var encryptedName = new char[_names.Length][];
        for (var i = 0; i < _names.Length; i++)
        {
            encryptedName[i] = _cast256Fpe.Encrypt(_names[i]);
        }

        return encryptedName;
    }

    [GlobalSetup(Target = nameof(DecryptNameCast256Fpe))]
    public void SetupDecryption()
    {
        ...
        {
            _names[i] = _cast256Fpe.Encrypt(names[i]);
        }
    }

    [Benchmark]
    public char[][] DecryptNameCast256Fpe()
    {
        var decryptedName = new char[_names.Length][];
        for (var i = 0; i < _names.Length; i++)
        {
            decryptedName[i] = _cast256Fpe.Decrypt(_names[i]);
        }
